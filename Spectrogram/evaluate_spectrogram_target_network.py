...
import numpy as np
import Spectrogram_CNN_Target_Network
import pickle
...
def calculate_pre_training(examples, labels, cnn):
    #examples, labels = load_data_spectrogram.read_data('D:\OneDrive\Doctorat\Bras Robotique\emgClassification\dataset')

    #datasets = [examples, labels]
    #pickle.dump(datasets, open("save_dataset_spectrogram.p", "wb"))

    X_training, Y_gesture, Y_human = [], [], []
    X_validation, Y_gesture_validation, Y_human_validation = [], [], []
    X_training_for_normalization = []
    human_number = 0
    for j in range(19):
        ...
        for k in range(len(examples[j])):
            if k < 21:
                #X_training_for_normalization.extend(examples[j][k])
    ...
    pickle.dump(weights_pre_training, open("pre_trained_final.p", "wb"))
...
def calculate_fitness(examples_training, labels_training, examples_validation0, labels_validation0,
    ...
    weight_pre_training = pickle.load(open("pre_trained_final.p"))
...
cnn = Spectrogram_CNN_Target_Network.CNN_Progressif(number_of_class=7, batch_size=1312, number_of_channel=4,
                                                    learning_rate=0.00681292, dropout=.35)
...
test_1 = []
for i in range(20):
    cnn = Spectrogram_CNN_Target_Network.CNN_Progressif(number_of_class=7, batch_size=128, number_of_channel=4,
...
with open("results_stft_transfer_learning_4_cycles_better_training_3.txt", "a") as myfile:
